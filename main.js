...
const autoRotateControl = document.getElementById('autoRotateControl');
const autoRotateValue = document.getElementById('autoRotateValue');
if (autoRotateControl && autoRotateValue) {
  const setAutoRotateSpeed = (speed) => {
    controls.autoRotateSpeed = speed;
    autoRotateValue.textContent = speed.toFixed(2);
  };

  setAutoRotateSpeed(parseFloat(autoRotateControl.value) || 0.3);

  autoRotateControl.addEventListener('input', (e) => {
...
const cancelEclipseTourBtn = document.getElementById('cancelEclipseTour');
if (cancelEclipseTourBtn) {
  cancelEclipseTourBtn.addEventListener('click', () => {
    stopEclipseTour();
  });
}
...
function onMouseClick(event) {
  ...
  planetMeshes.forEach((planetObj, planetIndex) => {
    if (planetObj.moons && planetObj.moons.length > 0) {
      planetObj.moons.forEach(moon => {
        ...
        clickableObjects.push(moon.mesh);
      });
    }
  });
  ...
  if (intersects.length > 0) {
    ...
    if (moonData) {
      ...
      // Show parent planet info so user can learn more, and scroll moons section
      showPlanetInfoCard(celestialBodies[moonData.planetIndex], moonData.planetIndex);
      return;
    }
...
// Change cursor on hover over interactive space objects (sun, planets, moons)
const hoverTargets = [sun];
planetMeshes.forEach(planetObj => {
  if (planetObj?.mesh) {
    hoverTargets.push(planetObj.mesh);
  }
  if (planetObj?.moons) {
    planetObj.moons.forEach(moon => {
      if (moon?.mesh) {
        hoverTargets.push(moon.mesh);
      }
    });
  }
});
